  ...
  return (
    ...
    >
          ...
          {activeIdx !== null && (
            ...
            >
               ...
               <div className="flex-1 flex items-center justify-center relative">
                 <span 
                   className="text-[220px] font-display font-black opacity-10 drop-shadow-2xl select-none"
               ...
               <div className="p-8 bg-gradient-to-t from-bg-primary via-bg-primary/80 to-transparent relative z-10 border-t border-white/5">
                 ...
                 <div className="flex gap-2 flex-wrap">
                   {projects[activeIdx].tech.slice(0, 3).map(t => (
                     <span key={t} className="px-2 py-1 bg-white/5 rounded-md border border-white/10 text-[9px] text-white/60 font-mono tracking-wider">
                       {t}
                     </span>
                   ))}
                 </div>
      ...
      <div className="max-w-[90rem] mx-auto px-6 md:px-12 relative z-10">
        <div className="mb-20 md:mb-32 flex flex-col md:flex-row md:items-end justify-between">
          <div>
            <p className="text-accent-violet text-[10px] font-heading tracking-[0.3em] uppercase mb-4">
              Featured Work
            </p>
          ...
          <div className="hidden md:flex items-center gap-4 text-white/20 font-mono text-sm">
        ...
        <div className="flex flex-col border-t border-white/10">
          {projects.map((project, i) => {
            ...
            return (
              <motion.div 
                key={project.id}
                className="border-b border-white/10 relative py-8 md:py-14 cursor-pointer"
                onMouseEnter={() => {
              ...
              >
                 ...
                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                   <div className="flex items-center gap-6 md:gap-16">
                     <span className={`font-mono text-sm md:text-xl transition-colors duration-500 ${isActive ? "text-white/40" : "text-white/10"}`}>
                       {String(i + 1).padStart(2, "0")}
                     </span>
                     <h3 
                       className={`text-4xl md:text-7xl lg:text-8xl font-display font-black transition-all duration-500 ${isActive ? "md:translate-x-6 md:italic" : ""}`}
                       style={{
                 ...
                 >
                    <div className="overflow-hidden">
                      <div className="pb-4">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {project.tech.map(t => (
                            <span key={t} className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-mono text-white/50 border border-white/10">
                              {t}
                            </span>
                          ))}
                        </div>
